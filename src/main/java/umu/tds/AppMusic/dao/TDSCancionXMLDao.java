 ...
 * Proporciona la funcionalidad para realizar operaciones CRUD sobre un archivo
 * XML de canciones.
 */
public class TDSCancionXMLDao implements CancionDao {
	...
	public void agregarCancion(Cancion cancion) {
		...
		// Normalmente, la adición se haría a una base de datos y no directamente al
		// XML.
	}
	...
	public void actualizarCancion(Cancion cancion) {
		// Similar a agregarCancion, la actualización normalmente no se hace
		// directamente en un archivo XML.
	}
	...
	public void eliminarCancion(Cancion cancion) {
		// La eliminación de datos en un archivo XML es poco común y depende de la
	...
	public Cancion obtenerCancionPorTitulo(String titulo) {
		...
		return cancionesXML.getCancion().stream().filter(cancionXML -> cancionXML.getTitulo().equalsIgnoreCase(titulo))
				.findFirst().map(this::convertirCancionXMLaCancion).orElse(null);
	}
	...
	public List<Cancion> obtenerTodasLasCanciones() {
		Canciones cancionesXML = MapperCancionesXMLtoJava.cargarCanciones(PATH_XML);
		return cancionesXML.getCancion().stream().map(this::convertirCancionXMLaCancion).collect(Collectors.toList());
	}
	...
	private Cancion convertirCancionXMLaCancion(CancionXml cancionXML) {
		...
		Cancion cancion = new Cancion(cancionXML.getTitulo(), cancionXML.getURL(), estilo.getNombre(),
